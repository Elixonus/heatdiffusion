...
def render(*, systems: list[System], show_conductivity: bool) -> None:
    ...
    temperatures_sort = sorted(temperature for temperatures_buffer in temperatures[0] for temperature in temperatures_buffer)
    ...
    for t, temperature in reversed(list(enumerate(temperatures))):
        system = systems[t]
        fig, ax = plt.subplots()
        im = ax.imshow(temperature, origin="lower", extent=(0, system.size[0], 0, system.size[1]), vmin=temperature_low, vmax=temperature_high, cmap="inferno", interpolation="bicubic")
